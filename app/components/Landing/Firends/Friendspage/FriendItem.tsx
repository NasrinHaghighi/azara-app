...
function FriendItem({ item, isOdd, backgroundImage }: any) {
    ...
    return (
        <>
        <div className="grid grid-cols-12 lg:px-24  py-8 text-text rounded-md mb-5 lg:gap-8 gap:0 border-2 "
            style={{
            ...
            <div className={`lg:col-span-5 col-span-12 flex flex-col justify-center lg:text-center border-b-2 border-bg  lg:border-b-0 lg:${isOdd ? 'order-1' : 'order-2'} order-1 `} >
                <div className='w-full  max-w-[150px] h-auto m-auto  flex justify-center items-center  '>
                    <Image src={item.img} alt="face" width={250} height={250} className="rounded-full mb-5 shadow-lg" />
                </div>
                <div>
                    <div className=" mb-3 flex flex-col items-center ">

                        <span className="text-green-700 font-semibold text-2xl">{item.name} </span>
            ...
            <div className={`lg:col-span-7 col-span-12   order-2 lg:${isOdd ? 'order-2' : 'order-1'} py-5 text-center `}>
                <div className='lg:text-3xl text-xl  font-semibold mb-4'>عنوان : <span className='text-green-500'>{item.title}</span></div>
                {/* poem */}
